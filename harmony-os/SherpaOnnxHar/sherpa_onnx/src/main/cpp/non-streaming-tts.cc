...
//   Added enableTimedPhonemes config flag reading in all create paths, and
//   a "phonemes" array in every generate result path when the flag is set.
//   The array itself is built by the shared BuildPhonemesArray() helper in
//   tts-playback.h -- not constructed inline here -- so there is exactly
//   one implementation of the {phoneme, id, startMs, endMs} mapping shared
//   with the additive PipeWire playback path (speak/startPlayback/
//   onRendererReady/stopSpeak/setOnComplete, registered by
//   RegisterTtsPlaybackExports() at the bottom of InitNonStreamingTts).
#include <algorithm>
#include <atomic>
#include <memory>
...
#include "tts-playback.h"  // BuildPhonemesArray() + RegisterTtsPlaybackExports()
...
static Napi::Value CreateOfflineTtsAsyncWrapper(
  ...
  // Phoneme timing adaptation: enables the "phonemes" array below.
  if (o.Has("enableTimedPhonemes") && o.Get("enableTimedPhonemes").IsBoolean()) {
    c.enable_timed_phonemes = o.Get("enableTimedPhonemes").As<Napi::Boolean>().Value();
  }
...
static Napi::Object OfflineTtsGenerateWithConfigWrapper(
  ...
  // Phoneme timing adaptation: see tts-playback.h for BuildPhonemesArray().
  if (audio->num_phonemes > 0) {
    result.Set("phonemes", BuildPhonemesArray(env, audio));
  }
...
static Napi::Object OfflineTtsGenerateWrapper(const Napi::CallbackInfo &info) {
  ...
  if (enable_external_buffer) {
    ...
    // Phoneme timing adaptation: see tts-playback.h for BuildPhonemesArray().
    if (audio->num_phonemes > 0) {
      ans.Set("phonemes", BuildPhonemesArray(env, audio));
    }
...
class TtsGenerateWorker : public Napi::AsyncWorker {
 ...
 protected:
  ...
  void OnOK() override {
    ...
    // Phoneme timing adaptation: see tts-playback.h for BuildPhonemesArray().
    if (audio_->num_phonemes > 0) {
      ans.Set("phonemes", BuildPhonemesArray(env, audio_));
    }
...
void InitNonStreamingTts(Napi::Env env, Napi::Object exports) {
  ...
  // Additive, non-blocking PipeWire playback path -- see tts-playback.cc.
  RegisterTtsPlaybackExports(env, exports);
}
