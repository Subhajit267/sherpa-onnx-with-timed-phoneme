...
//   Added enableTimedPhonemes config flag reading and phoneme array
//   construction in all generate result paths.  When the flag is set,
//   the returned JS object includes a "phonemes" array of timed phoneme
//   objects for lip‑sync.
#include <algorithm>
#include <atomic>
#include <memory>
...
static Napi::Object OfflineTtsGenerateWithConfigWrapper(
  ...
  if (audio->num_phonemes > 0) {
    Napi::Array phonemes = Napi::Array::New(env, audio->num_phonemes);
    for (int32_t i = 0; i < audio->num_phonemes; ++i) {
      Napi::Object obj = Napi::Object::New(env);
      obj.Set("phoneme", Napi::String::New(env, audio->phonemes[i].phoneme));
      obj.Set("id", Napi::Number::New(env, audio->phonemes[i].id));
      obj.Set("startMs", Napi::Number::New(env, audio->phonemes[i].start_second * 1000.0));
      obj.Set("endMs", Napi::Number::New(env, audio->phonemes[i].end_second * 1000.0));
      phonemes[i] = obj;
    }
    result.Set("phonemes", phonemes);
  }
  // =================================================
 ...
 protected:
  ...
  void OnOK() override {
    ...
    if (audio_->num_phonemes > 0) {
      Napi::Array phonemes = Napi::Array::New(env, audio_->num_phonemes);
      for (int32_t i = 0; i < audio_->num_phonemes; ++i) {
        Napi::Object obj = Napi::Object::New(env);
        obj.Set("phoneme", Napi::String::New(env, audio_->phonemes[i].phoneme));
        obj.Set("id", Napi::Number::New(env, audio_->phonemes[i].id));
        obj.Set("startMs", Napi::Number::New(env, audio_->phonemes[i].start_second * 1000.0));
        obj.Set("endMs", Napi::Number::New(env, audio_->phonemes[i].end_second * 1000.0));
        phonemes[i] = obj;
      }
      ans.Set("phonemes", phonemes);
    }
